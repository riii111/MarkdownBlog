...
func TestCreateArticle(t *testing.T) {
	...
	t.Run("正常系: 記事の作成成功", func(t *testing.T) {
		...
		// ステータスコードの検証なのだ
		assert.Equal(t, http.StatusCreated, w.Code, "記事作成は成功するべきなのだ")

		// レスポンスの検証なのだ
		var response dto.CreateArticleResponse
		e2e.GetResponseJSON(t, w, &response)

		assert.NotEmpty(t, response.Slug, "記事のスラグは空ではないはずなのだ")
	})
...
func TestDeleteArticle(t *testing.T) {
	...
	t.Run("正常系: 記事の削除成功", func(t *testing.T) {
		...
		// ステータスコードの検証なのだ
		assert.Equal(t, http.StatusNoContent, w.Code, "記事削除は成功するべきなのだ")
	})
...
func TestGetArticles(t *testing.T) {
	...
	t.Run("正常系: 記事一覧の取得", func(t *testing.T) {
		...
		// ステータスコードの検証なのだ
		assert.Equal(t, http.StatusOK, w.Code, "記事一覧取得は成功するべきなのだ")

		// レスポンスの検証なのだ
		var response dto.ArticleListResponse
		e2e.GetResponseJSON(t, w, &response)

		// 初期状態では記事がないかもしれないが、レスポンス構造は正しいはずなのだ
		assert.NotNil(t, response.Data, "データフィールドはnilではないはずなのだ")
		assert.NotNil(t, response.Pagination, "ページネーションフィールドはnilではないはずなのだ")
	})
	...
	t.Run("正常系: リミット指定での記事一覧取得", func(t *testing.T) {
		...
		assert.Equal(t, 5, response.Pagination.ItemsPerPage, "1ページあたりのアイテム数は5のはずなのだ")
	})
}
...
func TestGetArticleBySlug(t *testing.T) {
	...
	t.Run("正常系: 記事詳細の取得", func(t *testing.T) {
		...
		// ステータスコードの検証なのだ
		assert.Equal(t, http.StatusOK, w.Code, "記事詳細取得は成功するべきなのだ")

		// レスポンスの検証なのだ
		var response dto.ArticleDetailResponse
		e2e.GetResponseJSON(t, w, &response)

		assert.Equal(t, articleSlug, response.Data.Slug, "取得した記事のスラグは一致するはずなのだ")
	})
...
func TestGetMeArticles(t *testing.T) {
	...
	t.Run("正常系: 自分の記事一覧の取得", func(t *testing.T) {
		...
		// ステータスコードの検証なのだ
		assert.Equal(t, http.StatusOK, w.Code, "自分の記事一覧取得は成功するべきなのだ")
		...
		assert.NotNil(t, response.Data, "データフィールドはnilではないはずなのだ")
		assert.GreaterOrEqual(t, len(response.Data), 1, "少なくとも1つの記事があるはずなのだ")
	})

	t.Run("異常系: 認証なしで自分の記事一覧取得", func(t *testing.T) {
		// 認証なしで自分の記事一覧取得リクエストなのだ
...
func TestGetArticlesByTag(t *testing.T) {
	...
	t.Run("異常系: 存在しないタグの記事一覧取得", func(t *testing.T) {
		...
		// ステータスコードの検証なのだ
		assert.Equal(t, http.StatusNotFound, w.Code, "存在しないタグの記事一覧取得は404を返すべきなのだ")

		// 注: レスポンスの形式はAPIの実装によって異なる可能性があるため、
		// 単純にステータスコードだけを検証するのだ
	})
}
